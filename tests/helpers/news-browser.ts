...
const file=join(root,'latest.json');writeFileSync(file,JSON.stringify({generated_at:new Date().toISOString(),latest:['mac.dmg','linux.tar.gz'].map((file,i)=>({source:'github_releases',metric:'asset_downloads',value:i+3,observed_at:new Date().toISOString(),dimensions:{tag:'fixture',file}})),event_daily:[{day:'2026-09-14',source:'server',kind:'auth_request',events:3},{day:'2026-09-14',source:'server',kind:'profile_login',events:2}],source_runs:['github_releases','account_service','first_party_events'].map(source=>({source,status:'ok',finished_at:new Date().toISOString()}))}));
const mail:string[]=[];
