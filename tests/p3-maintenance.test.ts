...
test('P3 maintenance really expires traces, keeps durable feedback and reports verified backup',()=>{
 ...
 expect(db.query("SELECT 1 FROM recall_traces WHERE id='p3-trace'").get()).toBeNull();expect(result.report.backup).toMatchObject({verified:true,schema:43});
 const backups=fs.readdirSync(env.BACKUP_DIR).filter(n=>n.startsWith('qoopia-'));expect(backups.length).toBeGreaterThan(0);
 for(const name of backups){const folder=path.join(env.BACKUP_DIR,name);expect(fs.statSync(folder).mode&0o777).toBe(0o700);expect(verifyBackup(folder).schema).toBe(43);expect(fs.statSync(path.join(folder,'snapshot.db')).mode&0o777).toBe(0o600);}
});
