"""P3 copy of accepted P2 per-file isolation runner; no historical evidence writes."""
...
"""
import os,pathlib,subprocess,time,json,signal,concurrent.futures,tempfile,sys
root=pathlib.Path.cwd();out=root/'artifacts/p3/test-files';out.mkdir(parents=True,exist_ok=True)
files=sorted(str(p) for p in (root/'tests').glob('*.test.ts'))
def run(file):
 start=time.time();name=pathlib.Path(file).name
 with tempfile.TemporaryDirectory(prefix='p3-gate-') as tmp,(out/(name+'.log')).open('w') as log:
