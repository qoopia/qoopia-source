...
export function desktopRelease(publicPem:string,build:number){
 ...
 return desktopReleaseSchema.parse({format:'qoopia-desktop-release/1',build,version:'5.0.2',public_ed_key:Buffer.from(jwk.x,'base64url').toString('base64'),feed_url:'https://qoopia.ai/updates/macos/appcast.xml'});
}
