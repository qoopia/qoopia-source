...
// Fill the exact approved Graphite path; do not redraw it as a stroked monogram.
...
while i < tokens.count {
    ...
    else if command == "L" { path.addLine(to: CGPoint(x: number(), y: number())) }
    else if command == "H" { path.addLine(to: CGPoint(x: number(), y: path.currentPoint.y)) }
    else if command == "V" { path.addLine(to: CGPoint(x: path.currentPoint.x, y: number())) }
    else if command == "Z" { path.closeSubpath() }
