...
Authenticated account email text opts out of Cloudflare email obfuscation using its [documented HTML comments](https://developers.cloudflare.com/waf/tools/scrape-shield/email-address-obfuscation/#prevent-cloudflare-from-obfuscating-email). Without that marker, edge-injected decoding conflicts with the strict page CSP and owners see placeholders. Authorization and HTML escaping remain in place; the zone-wide setting and CSP are unchanged. Verify visible addresses in an existing owner browser session after rollout.

## Unified owner view

The owner page additionally presents all accessible Qoopia GitHub repositories, daily history, source coverage, collected service metrics and browser/server event aggregates. Repository collection is an hourly Mac job; its report timestamp, not the five-minute import timestamp, determines freshness. `scripts/analytics-github.py` must be installed alongside `analytics-providers.py` in the existing private Mac collector directory. No provider credentials move to Corsair. The server collector accepts `github_ecosystem` and exports only to the existing owner-only JSON mount. Existing owner authorization, email consent and sender configuration are preserved.
