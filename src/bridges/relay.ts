...
// The public invite page runs exactly this script and nothing else: its hash is the whole script-src.
const INVITE_SCRIPT=`const code=location.hash.slice(1);document.querySelector('#invite').value=/^QPB1\\.[A-Za-z0-9_-]{43}\\.[A-Za-z0-9_-]{43}$/.test(code)?code:'Invalid invitation';document.querySelector('#copy').onclick=async()=>{try{await navigator.clipboard.writeText(code);document.querySelector('#status').textContent='Copied. Paste into your Qoopia.'}catch{document.querySelector('#invite').select()}};`;
const INVITE_SCRIPT_HASH='sha256-'+new Bun.CryptoHasher('sha256').update(INVITE_SCRIPT).digest('base64');
