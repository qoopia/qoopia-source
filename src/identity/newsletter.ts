...
export function newsMessage(campaign:{subject:string;body:string;language:LoginLanguage},unsubscribe:string,postalAddress:string){
 ...
 return {subject:campaign.subject,text:`${campaign.body}\n\n${footer}\n${label}: ${unsubscribe}\nQoopia · ${postalAddress}`,
  html:`<!doctype html><html lang="${campaign.language}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="dark"></head><body style="margin:0;background:#111110;color:#F2EFE9"><table role="presentation" width="100%" bgcolor="#111110"><tr><td style="padding:40px 24px"><div style="max-width:560px;margin:auto;font:16px/1.6 'Manrope',Arial,sans-serif"><img src="cid:qoopia-brand" width="200" height="50" alt="qoopia" style="display:block;max-width:100%;height:auto;border:0;margin:0 0 40px"><h1 style="font-size:28px;line-height:1.25;font-weight:500;letter-spacing:-.02em">${e(campaign.subject)}</h1>${campaign.body.split(/\n\s*\n/).map(v=>`<p style="color:#F2EFE9;overflow-wrap:anywhere">${e(v).replace(/\n/g,'<br>')}</p>`).join('')}<hr style="border:0;border-top:1px solid #343330;margin:40px 0"><p style="color:#C3BDB4;font-size:14px">${footer}<br><a style="color:#F2EFE9" href="${e(unsubscribe)}">${label}</a><br>Qoopia · ${e(postalAddress)}</p></div></td></tr></table></body></html>`,
