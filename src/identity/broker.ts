...
type Config = {origin:string;resendKey:string;from:string;googleClientId:string;googleClientSecret:string;devices?:DeviceRegistryOptions;recordEvent?:RecordEvent;browserWrite?:(raw:unknown)=>boolean};
...
export function loginBroker(db: Database, config: Config, request: typeof fetch = fetch) {
  ...
  const profile=profilePortal(db,origin,page,(route,body,ip)=>handler(new Request(origin+route,{method:'POST',headers:{origin,'content-type':'application/json'},body:JSON.stringify(body)}),ip),record);
