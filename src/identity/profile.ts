...
export function profilePortal(db:Database,origin:string,page:Page,call:Call,record:RecordEvent=noAnalytics) {
  ...
  return {cleanup,handler:async(req:Request,ip:string):Promise<Response>=>{
    ...
    if(req.method==='GET'&&url.pathname==='/profile'){
      ...
      return profileView(page,ru,account,suggested,!!db.query('SELECT hash FROM profile_pending WHERE hash=?').get(hash(cookie(req,pendingName))));
    }
    ...
    try{
      ...
      if(url.pathname==='/profile/poll'){
        ...
        const pending=db.query('SELECT * FROM profile_pending WHERE hash=?').get(key) as {request_id:string;verifier:string}|null;
