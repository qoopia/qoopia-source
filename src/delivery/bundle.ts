...
export function verifyBundle(root: string, publicKey: string, allowTest = false, target = `${process.platform}-${process.arch}`) {
  ...
  // Published bundles through schema 44 keep this script inline in dashboard.html.
  // The same verifier checks the installed source during upgrade, not only the target.
  if(m.schema_max>=45&&!m.members['assets/src/public/brand/dashboard.js'])throw new Error('Required dashboard script missing');
