...
window.QoopiaChat = function ({api, esc, humanSize, onUnauthorized}) {
  ...
  function mount(host) {
    ...
    async function loadFiles(){
      if(filesLoading||!$('#agentFileDetails')?.open||filesLoadedAt&&Date.now()-filesLoadedAt<30000)return;
      filesLoading=true;$('#agentFileStatus').textContent=QI.msg('Loading…');
      try{
        const result=await api('/api/dashboard/my-agent?files=1&runs=1');if(!here())return;
        const files=result.files||[];
        $('#agentFileList').innerHTML=files.map(f=>'<li><a href="/api/dashboard/my-agent/file?path='+encodeURIComponent(f.path)+'" download>'+esc(f.path)+'</a> · '+humanSize(f.size)+'</li>').join('');
        $('#agentFileStatus').textContent=files.length?'':QI.msg('No files in this folder yet.');filesLoadedAt=Date.now();
      }catch(error){if(here())$('#agentFileStatus').textContent=error.message;}
      finally{filesLoading=false;}
    }
    async function load(){
      if(!here()||root.dataset.loading)return;root.dataset.loading='true';const revision=actionRevision;
      try{
        const next=await api('/api/dashboard/my-agent?runs=10&files=0');if(!here()||revision!==actionRevision)return;
        if(data?.active_conversation&&!next.active_conversation){filesLoadedAt=0;if($('#agentFileDetails')?.open)void loadFiles();}
        data=next;
        const preparing=data.operation?.state==='running';
        $('#agentSetupStep').textContent=data.access_error?QI.pair('Agent access needs attention','Проверьте доступ агента'):!data.configured?data.steward&&!(data.adoptable_providers||[]).length&&!data.can_adopt?'':QI.pair('1 of 3 · Choose a subscription','1 из 3 · Выберите подписку'):preparing?QI.pair('2 of 3 · Preparing your agent','2 из 3 · Подготавливаем агента'):!data.running?QI.pair('2 of 3 · Start your agent','2 из 3 · Запустите агента'):!data.account?QI.pair('3 of 3 · Sign in to your subscription','3 из 3 · Войдите в подписку'):QI.pair('Ready in dashboard · Telegram is optional','Готов в дашборде · Telegram по желанию');
        ...
        $('#agentSend').disabled=busy||preparing||!data.account||!!data.active_conversation||!!data.selected_provider&&data.selected_provider!==data.provider;$('#agentStop').hidden=!(preparing||data.active_conversation||data.telegram_setup?.queued);$('#agentStop').disabled=!(preparing||data.active_conversation||data.telegram_setup?.queued);
        ...
        const progressNames={commandExecution:QI.pair('Running a command…','Выполняет команду…'),Bash:QI.pair('Running a command…','Выполняет команду…'),fileChange:QI.pair('Editing a file…','Изменяет файл…'),Edit:QI.pair('Editing a file…','Изменяет файл…'),Write:QI.pair('Writing a file…','Записывает файл…'),Read:QI.pair('Reading a file…','Читает файл…'),Glob:QI.pair('Finding files…','Ищет файлы…'),Grep:QI.pair('Searching files…','Ищет в файлах…'),mcpToolCall:QI.pair('Using a connected tool…','Использует подключённый инструмент…')};
        $('#agentProgress').textContent=data.active_conversation?data.approvals.length?QI.pair('Your agent needs your answer.','Агенту нужен ваш ответ.'):progressNames[data.progress]||QI.pair('Your agent is working…','Агент работает…'):QI.msg(data.account?'Ready for your next message.':'Start your agent and sign in to continue.');
        ...
        if(!$('#agentFileDetails')){$('#agentFiles').innerHTML='<details id="agentFileDetails"><summary>'+esc(QI.msg('Files in your agent folder'))+'</summary><p id="agentFileStatus"></p><ul id="agentFileList"></ul></details>';$('#agentFileDetails').ontoggle=()=>{if($('#agentFileDetails').open)void loadFiles();};}
        if($('#agentFileDetails').open&&!filesLoading&&(!filesLoadedAt||Date.now()-filesLoadedAt>30000))void loadFiles();
        const tg=data.telegram;$('#telegramForm').hidden=!!tg.username;
        const connectedHtml=tg.username?'<p>'+esc('@'+tg.username)+' · '+esc(QI.msg(tg.verified?'Reply verified':tg.linked?'Linked. Send a message to verify the reply.':data.telegram_setup?.pending?.user?'Confirm your Telegram account':data.telegram_setup?.expired?'Pairing link expired. Create a new link.':'Open the pairing link and press Start in Telegram.'))+'</p><button id="telegramDisconnect">'+esc(QI.msg('Disconnect Telegram'))+'</button>':'';
        ...
        const telegramProblem=data.telegram_setup?.error?QI.msg(data.telegram_setup.error):data.telegram_setup?.uncertain_deliveries?QI.msg('A Telegram reply could not be confirmed. Your full answer is saved in this chat.'):'';
        if(telegramProblem)feedback(telegramProblem);else if(lastTelegramProblem&&$('#agentFeedback').textContent===lastTelegramProblem)feedback('');
        lastTelegramProblem=telegramProblem;
