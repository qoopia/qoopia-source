...
import {nativePackagePreview,nativeRuntimeEnvironment,unpackNativePackage,verifyInstalledNative,vendorDownload} from '../delivery/native-provision.ts';
...
type Login={state:'waiting'|'completed'|'failed';url?:string;code?:string;submit?:(code:string)=>void};
...
export function memorySetupState(ownerId:string) {
  ...
  return {model:memoryModelStatus(auth.workspace_id),embedding:embeddingCoverage(auth.workspace_id),sessions,busy:busy.has(auth.workspace_id),
...
export async function provision(runtime:MemoryProfile['runtime']) {
  ...
  if(fs.existsSync(selected)){nativeRuntimeEnvironment(root,{PATH:process.env.PATH});return;}
  ...
  if(fs.existsSync(dest))verifyInstalledNative(pkg,dest);
  else await unpackNativePackage(pkg,await vendorDownload(pkg.url,pkg.size),dest);
  durableWrite(selected,JSON.stringify(pkg));
}
function login(workspace:string,profile:MemoryProfile) {
  const home=privateDirectory(path.join(memoryRoot(),'native-login-home',workspace,profile.runtime));
  if(profile.runtime==='claude_code')prepareNativeKeychain(home);
  const environment=nativeRuntimeEnvironment(memoryRoot(),{PATH:process.env.PATH});
  ...
  const finish=(success:boolean)=>{clearTimeout(timer);clearTimeout(hard);pending='';logins.set(workspace,{state:success?'completed':'failed'});busy.delete(workspace);};
...
export async function memorySetupAction(ownerId:string,raw:unknown) {
  ...
  try {
    ...
    if(input.action==='login'){login(workspace,profile);return {state:'waiting'};}
